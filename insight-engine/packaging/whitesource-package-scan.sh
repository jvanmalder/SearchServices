...
then      
    POM_VERSION=$(grep version pom.xml | grep -v -e '<?xml version' | head -n 1 |awk -F '[><]' '{print $3}')
    RELEASE_FOLDER=/data/releases/InsightEngine/${bamboo_release_version}
    DISTRIBUTION_NAME=alfresco-insight-engine-distribution-${POM_VERSION}.zip
